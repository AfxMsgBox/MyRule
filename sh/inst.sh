...
#   mkdir -p /opt/myproxy && cd $_ && sh inst.sh        # 装到 /opt/myproxy/{sh,core,agh,sys}
#   sh inst.sh /etc/proxy                                # 显式指定，装到 /etc/proxy/{sh,core,agh,sys}
...
DIR_SYS="$DIR_INST/sys"           # 服务文件统一先落在此，再按用户选择决定是否安装到 /etc
mkdir -p "$DIR_SH" "$DIR_CORE" "$DIR_AGH" "$DIR_SYS"
...
export _DEPS_UPDATED=1                    # 子脚本不再重复下 env.conf / common.sh
...
# === 下载服务文件到 $DIR_SYS（先把 /etc/proxy 占位换成实际 $DIR_INST，再决定是否上 /etc） ===
case "$MP_OS_TYPE" in
    openwrt) sys_rels="init.d/proxy_core init.d/agh hotplug.d/net/99-meta-route" ;;
    systemd) sys_rels="systemd/system/proxy_core.service systemd/system/agh.service" ;;
esac
echo_log ">>> 下载服务文件到 $DIR_SYS（路径替换 /etc/proxy → $DIR_INST）"
for rel in $sys_rels; do
    mkdir -p "$DIR_SYS/$(dirname "$rel")"
    download_file "$MP_REPO_RAW_URL/sh/etc/$rel" "$DIR_SYS/$rel" true 0 \
        || { echo_log "下载 $rel 失败"; exit 1; }
    sed -i "s|/etc/proxy|$DIR_INST|g" "$DIR_SYS/$rel"
    chmod +x "$DIR_SYS/$rel" 2>/dev/null || true
done
...
# === [6/6] 自启动询问 ===
# 默认 Y：把 $DIR_SYS 下文件复制到 /etc 对应位置并 enable+start
# n：保留在 $DIR_SYS，写入 README.md 说明手动安装步骤
prompt_to autostart "Y" "[6/6] 现在启用并启动系统服务？(Y/n)" \
    "Y = 把 $DIR_SYS 下文件复制到 /etc 对应位置，并 enable + start" \
    "n = 跳过，文件留在 $DIR_SYS/ 与同目录 README.md，你后续手动装"

# 生成 sys/README.md（无论选 Y/n 都写，作为手动恢复参考）
case "$MP_OS_TYPE" in
    openwrt)
        cat > "$DIR_SYS/README.md" <<EOF
# 手动安装 MyProxy 服务（OpenWrt）

本目录包含已替换路径的服务文件（路径前缀: $DIR_INST）。
inst.sh 询问"启用自启动"选 n 时落在此处，由你按下面步骤手动装：

\`\`\`sh
cp $DIR_SYS/init.d/proxy_core           /etc/init.d/proxy_core
cp $DIR_SYS/init.d/agh                   /etc/init.d/agh
cp $DIR_SYS/hotplug.d/net/99-meta-route  /etc/hotplug.d/net/99-meta-route
...
卸载：\`service agh stop; service proxy_core stop; service agh disable; service proxy_core disable\`，再删上面 cp 的三个文件。
EOF
        ;;
    systemd)
        cat > "$DIR_SYS/README.md" <<EOF
# 手动安装 MyProxy 服务（systemd）

本目录包含已替换路径的 unit 文件（路径前缀: $DIR_INST）。
inst.sh 询问"启用自启动"选 n 时落在此处，由你按下面步骤手动装：

\`\`\`sh
cp $DIR_SYS/systemd/system/proxy_core.service /etc/systemd/system/
cp $DIR_SYS/systemd/system/agh.service        /etc/systemd/system/
systemctl daemon-reload
systemctl enable --now proxy_core.service agh.service
\`\`\`

卸载：\`systemctl disable --now agh.service proxy_core.service\`，再删 /etc/systemd/system/{agh,proxy_core}.service 并 \`systemctl daemon-reload\`。
EOF
        ;;
esac
...
case "$autostart" in
    n|N|no|NO|No)
        echo_log ">>> 已跳过自启动；服务文件保留在 $DIR_SYS/"
        echo_log "    按 $DIR_SYS/README.md 步骤完成自启动安装"
        ;;
    *)
        echo_log ">>> 安装服务文件到 /etc 并启动"
        for rel in $sys_rels; do
            mkdir -p "/etc/$(dirname "$rel")"
            cp "$DIR_SYS/$rel" "/etc/$rel"
